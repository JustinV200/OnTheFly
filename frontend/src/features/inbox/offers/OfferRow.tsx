/* One offer in the owner's ranked list: who (visible only to the owner), how it arrived, scope covered, the monthly price,
   potential savings, and a chip per evidence check. The whole row opens the offer drawer; the name and the Details button
   are the keyboard targets for the same action. Every figure is the server's, measured on the scope version it answered. */
...
export function OfferRow({ offer, currentScopeVersionNumber, rank, onOpen }: OfferRowProps): JSX.Element {
    ...
    <tr className="ranked-offers__offer" onClick={open}>
      <td className="ranked-offers__challenger">
        <div className="ranked-offers__who">
          {rank === null ? null : <span className="ranked-offers__rank" title="Server rank: scope covered first, then price">#{rank}</span>}
          <button
      ...
      <td className="ui-num" data-label="Monthly">
        <span className="ranked-offers__price"><MoneyDisplay amountMinor={offer.normalized_price_minor} currency={offer.price_currency} /></span>
