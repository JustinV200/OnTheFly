/* Step 4 of the offer trace: the listing the scope was published on, its published price, visibility, and bidding mode.
   It is titled by the listing's title, else by category for a listing from before tasks, as its own market card is. */
...
export function ListingStep({ listing }: ListingStepProps): JSX.Element {
  ...
  return (
    <TraceStep
      ...
      step={4}
      title={`Listing: ${listing.title || categoryLabel(listing.category)}`}
    >
      <Stat
