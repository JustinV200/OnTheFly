/* The task-market card: one public listing shown like a market. Title and area first, what the business pays now as the
   big number (or "Price not disclosed"), one line of scope, then bidding mode, offers, and time left. Used by the
   marketplace feed, similar listings, the public profile, and the publish preview, so a preview is literally what a
   stranger sees. Built only from public projection fields; it never takes private expense data. */
...
import { describeBilling } from './describeBilling';
...
export interface MarketCardListing {
  ...
  // Null when the poster chose not to show a price.
  price_minor: number | null;
  price_currency: string;
  billing_cadence: string;
  bidding_mode: string;
  challenge_deadline: string | null;
  // A task's own title (new tasks and pieces); older listings have none and are titled by category.
  title?: string | null;
...
export function MarketCard({ listing, offerCount, href, action, extraMeta, priceCaption, headingLevel = 3 }: MarketCardProps): JSX.Element {
  ...
  const title = listing.title || categoryLabel(listing.category);
    ...
    <article className={href ? 'market-card market-card--linked' : 'market-card'}>
      <header className="market-card__header">
        ...
        <div className="market-card__heading">
          ...
          <p className="market-card__area">{listing.title ? `${categoryLabel(listing.category)} · ${area}` : area}</p>
        </div>
      </header>

      <div className="market-card__price">
        {listing.price_minor === null ? (
          <span className="market-card__undisclosed">Price not disclosed</span>
        ) : (
          <>
      ...
      <p className="market-card__price-caption">
        {listing.price_minor === null ? `The business kept its price private; ${describeBilling(listing.billing_cadence)}` : priceCaption ?? 'Current price'}
      </p>
