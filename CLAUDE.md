...
- A live simulated fly brain (`features/brainview`, `shared/flybrain/live`). Every fly-brain result carries a `brain_stimulus` that a Web Worker plays on a leaky integrate-and-fire port of the FlyWire release 783 model, loaded from `frontend/public/flybrain`. It is a simulation shown alongside results; results don't depend on it, and it is not Fly Scout. The dock starts minimized to a labeled pill.
  - The one exception is **the fruit fly's opinion** (`features/brainview/opinion`, 2026-09-13): a speech bubble beside each offer on the task page and in the offer drawer. The inbox row carries a `fly_opinion_stimulus` (`services/comparison/fly_opinion_stimulus.py`): "what you pay now" then "this offer" as smells, the offer fainter the cheaper it is per month and stronger for each scope gap. The worker runs both trials and the bubble says Yes when the offer caused fewer spikes, with a 1–5 rating from how far the counts differ. It is labeled "Simulated fly brain · a toy, not advice", uses the fly-brain violet only, and never gates or performs acceptance. It knows nothing about a vendor's history or evidence.
