...
`python -m app.cli.seed_demo` drops/recreates the configured database and creates the earlier cleaning scenario. Stop the server and back up any important data first. Stored Stripe connections are removed. Its limited counteroffer preservation is not a full backup. Do not run this command as part of normal startup or for the new GovCon ledger.
...
Last Stripe implementation check: 39 backend tests passed, including mocked-provider ownership, pagination and transaction-status tests. The Windows test-engine cleanup was fixed. Real Stripe sandbox consent and the new REBID story still need their own acceptance runs.
