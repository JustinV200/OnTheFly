...
| 2 | "Every expense starts private." | Point at the summary: **5 private, 0 public**. Every row shows 🔒 Private. Payroll shows "never publishable". |
...
| 10 | Shortlist | **Not built** (cut to [../10-stretch.md](../10-stretch.md)). Say "the owner would shortlist here" and move on. Don't mime a button. |
...
- **"Is that real data?"** Point at the strip and the `fixture · demo data` badges. Fixtures, labeled everywhere, because the Rho adapter isn't verified yet.
...
- Financial data is fixture data, not a live Rho connection.
